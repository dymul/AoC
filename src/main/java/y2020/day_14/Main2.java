...
public class Main2 {
    ...
    private static void mem(String s) {
        String[] split = s.split(" = ");
        long value = Long.parseLong(split[1]);
        Long address = Long.parseLong(split[0].replace("mem[", "").replace("]", ""));
        Long mask1 = Long.parseLong(mask.replaceAll("X", "0"),2);
        address = address | mask1;
        String mask0_rev = new StringBuilder(mask.replaceAll("1", "0")).reverse().toString();
        put(address, mask0_rev, value);

    }
    private static void put(long address, String mask, Long value) {
        if(!mask.contains("X")) {
            ...
            long mask0 = 1L << pos;
            put(address | mask0, mask.replaceFirst("X", "0"), value);
            long mask1 = ~mask0;
            put(address & mask1, mask.replaceFirst("X", "0"), value);
        }
    }
