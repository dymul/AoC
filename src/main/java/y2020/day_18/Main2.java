...
public class Main2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < in.length; i++) {
            String[] split = in[i].replaceAll(" ", "").split("");
            Queue<String> q = new LinkedList<>();
            for(String s : split) {
                 q.add(s);
            }
            long eval = eval(q);
            System.out.println(in[i] +" = " + eval);
            sum +=eval;
        }
        System.out.println("Total: " + sum);
    }

    private static long eval(Queue<String> s) {
        Deque<Long> operands = new LinkedList<>();
        operands.addFirst(getOperand(s));
        while (!s.isEmpty()) {
            Operator o = Operator.fromString(s.remove());
            long operand = getOperand(s);
            if (o == Operator.ADD) {
                long result = o.eval(operands.removeLast(), operand);
                operands.add(result);
            } else {
               operands.add(operand);
            }
        }
        long result = operands.removeFirst();
    ...
    private static long getOperand(Queue<String> s) {
        String first = s.remove();
        if (!first.equals("(")) {
            ...
            while (brackets > 0) {
                ...
                if (next.equals("(")) brackets++;
                if(next.equals(")")) brackets--;
                if(!(next.equals(")") && brackets==0)) {
                    q1.add(next);
                }
            }
            return eval(q1);
