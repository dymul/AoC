...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(parents(entries.get("shiny gold")));
        System.out.println(children(entries.get("shiny gold")));
    }
    ...
    static void parse(String input) {
        ...
        for (int i = 4; i  + 3 < split.length; i+=4) {
            ...
            String child_color = split[i+1] + " " + split[i+2];
            Entry child = entries.get(child_color);
            current.children.add(new Child(child, number));
            if (child ==null) {
    ...
    private static long children(Entry entry) {
        long count = 1;
        for (Child n : entry.children) {
            count += (n.amount *children(n.entry));
        }
        return count;
    }

    private static int parents(Entry looked) {
        Set<Entry> parents = new HashSet<>();
        Deque<Entry> queue = new LinkedList<>();
        queue.addAll(looked.parents);
        while (queue.size()!= 0) {
