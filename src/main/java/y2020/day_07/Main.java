...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        parents();
        System.out.println(children(Entry.entries.get("shiny gold")));
    }

    private static long children(Entry entry) {
        long count = 1;
        for (Neigbour n : entry.neighbours) {
            count += (n.number*children(n.entry));
        }
        return count;
    }

    private static void parents() {
        Entry shiny_gold = Entry.entries.get("shiny gold");
        Set<Entry> parents = new HashSet<>();
        Deque<Entry> queue = new LinkedList<>();
        for (Entry n : shiny_gold.parents) {
            queue.add(n);
        }
        while (queue.size()!= 0) {
            Entry entry = queue.removeFirst();
            parents.add(entry);
            for (Entry n : entry.parents) {
                queue.add(n);
