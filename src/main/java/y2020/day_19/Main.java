...
public class Main {
    ...
    static Map<Integer, Rule> map = new HashMap<>();
    ...
    public static void main(String[] args) {
        ...
        map.get(42).solve(map);
        map.get(31).solve(map);
        System.out.println(map.get(42).matchers);
        System.out.println(map.get(31).matchers);

        int matches = 0;
        for(String word : words) {
            String[] parts = new String[word.length()/8];
            for (int i = 0; i < parts.length; i++) {
                parts[i] = word.substring(i*8, i*8+8);
            }
            int frontMatch = 0;
            for (frontMatch = 0; frontMatch < parts.length; frontMatch++) {
                if (!map.get(42).matchers.contains(parts[frontMatch])) {
                    break;
                }
            }
            int backMatch = 0;
            for (backMatch = 0 ; backMatch < parts.length; backMatch++) {
                if (!map.get(31).matchers.contains(parts[parts.length - 1 - backMatch])) {
                    break;
                }
            }
            if (backMatch >=1 &&
                frontMatch + backMatch >=parts.length &&
        ...
        //long count = Arrays.stream(words).filter(w -> map.get(0).matchers.contains(w)).count();
        //System.out.println(count);

    }


    static String[] rules1 = {
            "0: 4 1",
            "1: 2 3 | 3 2",
            "2: 4 4 | 5 5",
            "3: 4 5 | 5 4",
