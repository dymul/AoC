...
public class Day10 implements Day {
    ...
    private String transform(String s) {
        ...
        while(i < letters.length) {
            int j = i+1;
            while (j < letters.length && letters[j].equals(letters[i])) {
                j++;
            }
            int count = j-i;
    ...
    public String solve2(List<String> input) throws Exception {
        String firstAtom = valueMap.entrySet()
                .stream()
                .filter(e -> e.getValue().equals(input.get(0)))
                .map(Map.Entry::getKey)
                .findFirst()
                .orElseThrow(IllegalArgumentException::new);
        List<String> seq = Collections.singletonList(firstAtom);
        for (int i = 0; i < 50; i++) {
            seq = transformSequence(seq);
        }
        int sum = seq.stream().mapToInt(s -> valueMap.get(s).length()).sum();

        return "" + sum;
    }

    private List<String> transformSequence(List<String> seq) {
        List<String> newSeq = new ArrayList<>();
        for (String s : seq) {
            newSeq.addAll(transitionMap.get(s));
        }
        return newSeq;
    }


    private static final String[][] transitionTable = {{"1","H","H","22","22"},
    ...
    private static final Map<String, List<String>> transitionMap = Arrays.stream(transitionTable)
            .collect(Collectors.toMap(array -> array[1], array -> Arrays.asList(array[2].split(" "))));
    private static final Map<String, String> valueMap = Arrays.stream(transitionTable)
            .collect(Collectors.toMap(array -> array[1], array -> array[3]));
